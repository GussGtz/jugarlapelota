  ...
  <div class="space-y-4 md:space-y-6">
    ...
    <div v-if="selTournament" class="bg-gradient-to-r from-primary/5 to-sky-50 border border-primary/20 rounded-2xl p-4 space-y-3">
      ...
      <div class="flex items-center gap-2">
        ...
        <p class="font-bold text-slate-900 text-sm">Link de Pre-inscripción</p>
      </div>
      <p class="text-slate-500 text-xs">
        Comparte con equipos interesados en <strong>{{ selTournament.name }}</strong>. Ellos llenan el formulario y tú apruebas o rechazas.
      </p>

      <!-- URL box -->
      <div class="flex items-center gap-2 bg-white border border-slate-200 rounded-xl px-3 py-2.5">
        <span class="text-slate-600 text-xs font-mono flex-1 truncate select-all min-w-0">{{ inscriptionUrl }}</span>
        <button @click="copyLink"
          class="flex items-center gap-1 text-xs font-bold px-2.5 py-1.5 rounded-lg transition-all shrink-0"
      ...
      <div class="flex gap-2">
        <a :href="whatsappUrl" target="_blank" rel="noopener"
          class="flex-1 inline-flex items-center justify-center gap-1.5 text-xs font-bold px-3 py-2 bg-[#25D366]/10 text-[#128C4A] border border-[#25D366]/30 rounded-lg hover:bg-[#25D366]/20 transition-colors">
        ...
        <a :href="inscriptionUrl" target="_blank" rel="noopener"
          class="flex-1 inline-flex items-center justify-center gap-1.5 text-xs font-bold px-3 py-2 bg-slate-100 text-slate-600 border border-slate-200 rounded-lg hover:bg-slate-200 transition-colors">
          <IconExternalLink class="w-3.5 h-3.5 shrink-0" />
          Abrir formulario
        </a>
      </div>
    </div>
