    ...
    <div v-if="selTournament" class="link-card mb-4">
      ...
      <div class="flex items-center gap-2 mb-2">
        <div class="w-7 h-7 rounded-lg bg-primary/10 flex items-center justify-center shrink-0">
          <IconLink class="w-3.5 h-3.5 text-primary" />
        </div>
        <p class="font-bold text-slate-900 text-sm">Link de Pre-inscripción</p>
      </div>
      <p class="text-slate-500 text-xs mb-3">
        Comparte con equipos de <strong>{{ selTournament.name }}</strong> para que se inscriban.
      </p>

      <!-- URL + copiar -->
      <div class="url-row mb-3">
        <span class="url-text">{{ inscriptionUrl }}</span>
      ...
      <div class="grid grid-cols-2 gap-2">
        <a :href="whatsappUrl" target="_blank" rel="noopener" class="share-btn share-wa">
        ...
        <a :href="inscriptionUrl" target="_blank" rel="noopener" class="share-btn share-link">
...
.share-wa  { background: #dcfce7; color: #15803d; border: 1px solid #86efac; }
.share-link { background: #f1f5f9; color: #475569; border: 1px solid #e2e8f0; }
</style>
